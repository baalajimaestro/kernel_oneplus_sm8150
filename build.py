    ...
    try:
        ...
        if return_code != 0:
            ...
            for i in log[-3000:]:
                str1 += i
            nekobin_key = requests.post("https://nekobin.com/api/documents", json={"content": str1}).json()["result"]["key"]
            message_track = await edit_message(518221376, "[Build Log](https://nekobin.com/" + nekobin_key + ")", message_track)
            raise RuntimeError

        message_track = await edit_message(518221376, "`Build Succeeded\!`", message_track)
